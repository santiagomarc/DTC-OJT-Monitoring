    ...
    <div className="overflow-hidden rounded-xl border border-gray-200 dark:border-white/10">
      <table className="w-full text-sm">
        ...
        <tbody className="divide-y divide-gray-200 dark:divide-white/5">
          {logs.map((log) => (
            <tr key={log.id} className="transition hover:bg-gray-50 dark:hover:bg-white/5">
              <td className="px-4 py-3 text-gray-900 dark:text-gray-300 whitespace-nowrap">{formatDate(log.date)}</td>
              <td className="px-4 py-3 text-gray-900 dark:text-gray-300 whitespace-nowrap">{formatTime(log.time_in)}</td>
              ...
              <td className="px-4 py-3 whitespace-nowrap">
                {log.total_hours != null ? (
                  <span className="font-medium text-violet-600 dark:text-violet-300">{Number(log.total_hours).toFixed(2)}h</span>
                ) : '—'}
              </td>
